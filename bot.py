import openai, telebot, datetime, os, time, re, requests
...
def generate_image(prompt, message):
    ...
    output_format='url'
    size='1024x1024'
    invalid_words = ['clevage','girl wearing','fuk','horny','no clothes','suck','copulation','twerking','scarlett','fck','narendra','gangbang','intercourse','stepmom','stepsister','xxx','xnxx','whore','ass','hot girl','underwear','girl bathing','xi jingping','modi','brazzers','biden','joe','salman','porn','doggy','mia khalifa','sunny leone','bathing','hot girl','inner','inner wear','sexy','boob', 'leah gotti','nipple','kiss','fuck', 'dick', 'sex','asshole', 'vagina', 'naked','penis', 'butt','breast', 'nude', 'titty', 'titties','poop']
...
def play_command(message):
    ...
    if len(query) == 0:
        ...
        if 'movie' in query.lower() or 'cinema' in query.lower() or 'film' in query.lower() or 'full movie' in query.lower() or '2023' in query.lower():
            ...
            global filename
            filename = f'{message.message_id}OO{message.chat.id}'
...
def callback_query_handler(call):
    if call.data.startswith('hehe'):
        ...
        try:
            ...
            if os.path.getsize(filename+'.mp3') >= 50000000:
                bot.send_message(chat_id=int(filename.split("OO")[1]), text="The filesize is too large! Subscribe to premium to get rid of restrictions!")
                ...
                with open(filename+'.mp3', 'rb') as audiofile:
                    bot.send_audio(chat_id=int(filename.split("OO")[1]),reply_to_message_id=int(filename.split("OO")[0]),audio=audiofile, performer=authorplay, title=titleplay, caption=caption, parse_mode='Markdown', reply_markup=markupclose)
                    bot.delete_message(chat_id=_message.chat.id, message_id=_message.message_id)
        ...
        markup = InlineKeyboardMarkup(row_width=2).add(InlineKeyboardButton("High Resolution", callback_data="highresolution"), InlineKeyboardButton("Low resolution", callback_data="lowresolution"), InlineKeyboardButton("﹤﹤ Back", callback_data="ythomepage"))
        ...
        filenamevideo = f'{call.message.message_id}OO{call.message.chat.id}'
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="_Getting high resolution streams . . ._", parse_mode="Markdown")
        ydl_optsvideo = {
        ...
        with YoutubeDL(ydl_optsvideo) as ydl:
            ...
            try:
                ...
                if os.path.getsize(filenamevideo+'.ogg') >= 50000000:
                     bot.send_message(int(filenamevideo.split("OO")[1]), "Filesize is too large! Subscribe to premium to remove restrictions!")
                     bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
                else:
                    ...
                    with open(filenamevideo+'.ogg', 'rb') as videofile:
                        bot.send_video(chat_id=int(filenamevideo.split("OO")[1]),reply_to_message_id=int(filenamevideo.split("OO")[0]), video=videofile, caption=f"{title}\n\n[Morty AI](https://t.me/morty_ai_bot)", parse_mode="Markdown")
                        bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
    ...
    elif call.data == 'audiores':
        bot.edit_message_reply_markup(chat_id=call.message.chat.id, message_id=call.message.message_id, reply_markup=InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("Highest quality", callback_data="dwaudio"), InlineKeyboardButton("﹤﹤ Back", callback_data="ythomepage")))

    elif call.data == 'lowresolution':
        global filenamevideolow
        # filenamevideolow = f'videolow{str(randomNumber())}'
        filenamevideolow = f'{call.message.message_id}OO{call.message.chat.id}'
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="_Getting low resolution streams . . ._", parse_mode="Markdown")
        ydl_optsvideolow = {
        ...
        with YoutubeDL(ydl_optsvideolow) as ydl:
            ...
            try:
                ...
                if os.path.getsize(filenamevideolow+'.mp4') >= 50000000:
                    bot.send_message(int(filenamevideolow.split("OO")[1]), "The filesize is too large! Subscribe to premium to get of restrictions!")
                else:
                    html = requests.get(youtubevideourl).text
                    title = re.findall(r"<title>(.*?) - YouTube</title>", html)[0]
                    with open(filenamevideolow+'.mp4', 'rb') as videofile:
                        bot.send_video(chat_id=int(filenamevideolow.split("OO")[1]),reply_to_message_id=int(filenamevideolow.split("OO")[0]),video=videofile,  caption=f"{title}")
                        bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
            ...
            try:
                ...
                print("ERROR WHILE DELETING")
    elif call.data == 'ythomepage':
        homemarkup = InlineKeyboardMarkup(row_width=2).add(InlineKeyboardButton("Video 📹", callback_data="dwvideo"), InlineKeyboardButton("Audio 🎧", callback_data="audiores"), InlineKeyboardButton("❌ Cancel ❌", callback_data="close"))
        ...
        filenameaudio = f'{call.message.message_id}OO{call.message.chat.id}'
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            try:
                ...
                with open(filenameaudio+'.mp3', 'rb') as audiofile:
                    bot.send_audio(chat_id=int(filenameaudio.split("OO")[1]),reply_to_message_id=int(filenameaudio.split("OO")[0]),audio=audiofile, caption=caption, parse_mode='Markdown')
                    bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)
            except Exception as n:
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="❗️ ERROR WHILE SENTING")
            ...
            for i in json_data:
                if i['id'] == str(userID):
                    ...
                    if images_generated >= 2 and isSubscriber(userID) == 0:
                        ...
                        bot.send_message(message.chat.id, "Send `/subscribe` to see details on premium user (Note: It's cheap)", parse_mode='Markdown')
                    else:
...
def youtube_command(message):
    ...
    if len(query) == 0:
        ...
        if isValid(youtubevideourl):
            ...
            markup = InlineKeyboardMarkup(row_width=2).add(InlineKeyboardButton("Video 📹", callback_data="dwvideo"), InlineKeyboardButton("Audio 🎧", callback_data="audiores"), InlineKeyboardButton("❌ Cancel ❌", callback_data="close"))
...
def aiChatHandlerFunction(message):
    ...
    if len(query) == 0:
        ...
        with open('users.json', 'r') as file:
            ...
            for i in json_data:
                if i['id'] == str(userId):
                    messages_generated = i['messages_generated']
                    if messages_generated >= 4 and isSubscriber(userId) == 0:
                        ...
                        if ai_response == None:
                            ...
                            i['messages_generated'] = messages_generated
        with open('users.json', 'w') as writefile:
            json.dump(json_data, writefile, indent=4)
