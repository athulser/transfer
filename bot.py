...
def generate_image(query, message):
    # num_image = 1
    # output_format='url'
    # size='1024x1024'
                 ...
                 #    images = response['images']
                    try:
                     #    for image in images:
                        ...
                        bot.delete_message(message.chat.id, _message.message_id)
                        with open(directory+query+'/'+path[2], 'rb') as f:
                            bot.send_photo(_message.chat.id,photo=f,parse_mode='Markdown', caption="Generated by Morty AI\n\n\n[Join MortyLabz](https://t.me/mortylab) | [Donate me](https://buymeacoffee.com/mortylabz)")
        ...
        if query.count(':') >= 2:
            ...
            if int(lines) >= 8 and isSubscriber(id) == 0:
                ...
                if int(lines) >=11:
                    ...
                    try:
                        result = wikipedia.summary(query, sentences=int(lines))
                    except PageError:
