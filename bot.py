...
def generate_image(prompt, message):
    ...
    output_format='url'
    size='512x512'
    invalid_words = ['narendra','modi','biden','joe','salman','porn','doggy','mia khalifa','sunny leone','bathing','hot girl','inner','inner wear','sexy','boob', 'leah gotti','nipple','kiss','fuck', 'dick', 'sex','asshole', 'vagina', 'naked','penis', 'butt','breast', 'nude', 'titty', 'titties','poop']
...
def play_command(message):
    ...
    if len(query) == 0:
        ...
        if 'movie' in query.lower() or 'cinema' in query.lower() or 'film' in query.lower() or 'full movie' in query.lower() or '2023' in query.lower():
            ...
            global filename
            filename = f'{query.replace(" ", "")}{str(randomNumber())+".mp3"}'
...
def callback_query_handler(call):
    if call.data.startswith('hehe'):
        ...
        filenamevideolow = f'videolow{str(randomNumber())}.ogg'
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text="⌛️")
        ydl_optsvideolow = {
            "format":"worstvideo[ext=ogg]+worstaudio[ext=mp3]/worst",
