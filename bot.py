...
# def pfp(userid):
#     user_photos = bot.get_user_profile_photos(user_id=userid).photos
#     photos = []
#     for photo in user_photos:
#         photos.append(photo[0].file_id)
#     return photos
...
# def account(message):
#     status = 'Free user'
#     if isSubscriber(message.chat.id) == 1:
#         status = 'Premium user'
    
#     username = message.from_user.username
#     userID = message.chat.id
#     desc = f'*Username : {username}*\n*User ID : {str(userID)}*\n*Status : {status}*\n\n*_________________*'
#     markup = InlineKeyboardMarkup(row_width=2).add(InlineKeyboardButton("Wallet", callback_data="wallet"), InlineKeyboardButton("Statistics", callback_data="stats"))
#     photo_ids = pfp(userID)
    
#     bot.send_photo(userID, photo_ids[0], caption=desc, reply_markup=markup, parse_mode='Markdown')
...
def reset_data(message):
    if str(message.chat.id) == SUDO_ID:
        res = resetFile()
        if res == "200":
            bot.send_message(int(SUDO_ID), "users.json has been resetted!")
