...
def callback_query_handler(call):
    ...
    if call.data.startswith('hehe'):
        ...
        markup = InlineKeyboardMarkup(row_width=2).add(InlineKeyboardButton("High Resolution", callback_data=f"highres{payloadvideo}"), InlineKeyboardButton("Low resolution", callback_data=f"lowres{payloadvideo}"), InlineKeyboardButton("﹤﹤ Back", callback_data=f"home{payloadvideo}"))
            ...
            for i in json_data:
                if i['id'] == str(userID):
                    ...
                    if images_generated >= 2 and isSubscriber(userID) == 0:
                        ...
                        bot.send_message(message.chat.id, "Send /subscribe to see details on premium user (Note: It's cheap)")
                    else:
