import openai, telebot, datetime, os, time, re, requests, random
...
def callback_query_handler(call):
    if call.data.startswith('hehe'):
        if not isSubscriber(call.message.chat.id):
                ...
                if call.message.chat.id in active_users:
                    if time_difference(active_users[call.message.chat.id]):
                        ...
                        bot.send_message(call.message.chat.id, "You can request another download in %d seconds\n\n/subscribe to get rid of time limits :)"%(60-(now - active_users[call.message.chat.id])))
                        return
                else:
                    active_users[call.message.chat.id] = now
