...
# @bot.message_handler(commands=['instagram'], chat_types=['private', 'group', 'supergroup'])
# def instagram_command(message):
#     query = message.text.replace('/instagram', '').replace('@morty_ai_bot', '').strip()
#     userID = message.chat.id
#     if len(query) == 0:
#         bot.send_message(userID, "Use in `/instagram @username` format.", parse_mode='Markdown')
#     else:
#         def job():
#             if query.startswith('@'):
#                 username = query.replace('@', '').strip()
#                 if "og:title" not in requests.get('https://instagram.com/'+username).text:
#                     bot.send_message(chat_id=userID,reply_to_message_id=message.message_id, text=f"User `{username}` does not exist!", parse_mode='Markdown')
#                 else:
...
#                     content = f'''
# ⚡️ <code>Name</code> : <b>{full_name}</b>
# ⚡️ <code>User ID</code> : <b>{user_id}</b>
# ⚡️ <code>Followers</code> : <b>{followers}</b>
# ⚡️ <code>Followers</code> : <b>{followers}</b>
# ⚡️ <code>Following</code> : <b>{following}</b>
# ⚡️ <code>Total posts</code> : <b>{posts_count}</b>
# ⚡️ <code>Website URL</code> : {external_url}
# ⚡️ <code>Bio</code> :

# {bio}

# '''
#                     bot.send_photo(userID, photo=profile_url, caption=content, parse_mode='html', reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("❌ Close ❌", callback_data='close')))
...
#             else:
#                 bot.send_message(userID, 'Invalid format!\nSend in `/instagram @yourusername` format', parse_mode='Markdown')
            ...
            if not isIgLink(query):
                ...
                if query.startswith('https://www.instagram.com/reel/') or query.startswith('https://instagram.com/reel/'):
                    ...
                    if video == 'error':
                        ...
                        bot.delete_message(userID, _messag.message_id)
                        try:
                            bot.send_video(userID, video=video, caption=caption, parse_mode="Markdown")
                    ...
                    if len(medias) == 0:
                        ...
                        for i in medias:
                            if i.startswith('##photo##'):
                                try:
                                    bot.send_photo(userID, photo=i.replace('##photo##', '').strip(), caption=caption, parse_mode='Markdown')
                                except ApiTelegramException as exception:
                                    bot.send_message(userID, "Exception occured")
                                    towrite = {'URL':query, 'ERROR':str(exception)}
                                    igerrorlogs_collection.insert_one(towrite)
                                ...
                                try:
                                    bot.send_video(userID, video=i.replace('##video##', '').strip(), caption=caption, parse_mode='Markdown')
...
def callback_query_handler(call):
    if call.data.startswith('hehe'):
        ...
        text = '━━━━━ ᴜꜱᴀɢᴇ ━━━━━\n\n★ 1 ɪᴍᴀɢᴇ = 2 ᴄʀᴇᴅɪᴛꜱ\n\n★ ɪꜰ ʏᴏᴜ ᴀʀᴇ ᴀ ꜱᴜʙꜱᴄʀɪʙᴇʀ 20 ᴄʀᴇᴅɪᴛꜱ ᴡɪʟʟ ʙᴇ ᴀᴅᴅᴇᴅ ᴛᴏ ʏᴏᴜʀ ᴀᴄᴄᴏᴜɴᴛ ᴅᴀɪʟʏ'
