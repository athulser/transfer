...
def me(message: telebot.types.ChatMemberUpdated):
    ...
    if update.status == 'member':
        ...
        if message.chat.type in ['group', 'supergroup']:
            bot.send_message(message.chat.id, "Great!! Now make me admin to start working!")
...
def callback_query_handler(call):
    ...
    elif call.data.startswith('swel'):
        admin_id = call.data.split('_')[1].strip()
        chatID = call.message.chat.id 
        if call.from_user.id == int(admin_id):
            if call.data.replace('swel', '').split('_')[0].strip() == 'on':
                welcomemessage(chat_id=chatID, action='on')
            elif call.data.replace('swel', '').split('_')[0].strip() == 'off':
                welcomemessage(chat_id=chatID, action='off')
            
            bot.edit_message_text(chat_id=chatID, message_id=call.message.message_id, text='Changes saved ✅', reply_markup=InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("<< Back", callback_data=f"wel_{admin_id}"), InlineKeyboardButton("❌ Close ❌", callback_data=f'cls_{admin_id}')))
            bot.answer_callback_query(call.id, text="Settings updated ✅")
        else:
            bot.answer_callback_query(call.id, "Only admins can perform this action", show_alert=True)
    ...
    ############################# WELCOME MESSAGE SETTINGS - (STARTS HERE) #############################

    elif call.data.startswith('wel_'):
        admin_id = call.data.replace('wel_', '').strip()
        chatID = call.message.chat.id
        if call.from_user.id == int(admin_id):
            current_status_welcome = groups_collection.find_one({'id':str(chatID)})['general']['welcome']
            group_name_welcome = bot.get_chat(chatID).title
            if current_status_welcome == 'off':
                keys1 = [[InlineKeyboardButton("On", callback_data=f'onweloff_{admin_id}'), InlineKeyboardButton('Off', callback_data=f'ofweloff_{admin_id}')], [InlineKeyboardButton("Change messsage", callback_data=f'changewel_{admin_id}')], [InlineKeyboardButton("❌ Close ❌", callback_data=f'cls_{admin_id}')]]
                bot.edit_message_text(chat_id=chatID, message_id=call.message.message_id, text=f'<b>Welcome message settings for {group_name_welcome}</b>\n\n<i>If turned on, whenever a user joins the group, the bot will sent a welcome message. To change the current message, click the change message button</i>\n\n<b>Current status : Off</b> ❌', parse_mode='html', reply_markup=InlineKeyboardMarkup(keyboard=keys1))
            else:
                keys1 = [[InlineKeyboardButton("On", callback_data=f'onwelon_{admin_id}'), InlineKeyboardButton('Off', callback_data=f'ofwelon_{admin_id}')], [InlineKeyboardButton("Change messsage", callback_data=f'changewel_{admin_id}')], [InlineKeyboardButton("❌ Close ❌", callback_data=f'cls_{admin_id}')]]
                bot.edit_message_text(chat_id=chatID, message_id=call.message.message_id, text=f'<b>Welcome message settings for {group_name_welcome}</b>\n\n<i>If turned on, whenever a user joins the group, the bot will sent a welcome message. To change the current message, click the change message button</i>\n\n<b>Current status : On</b> ✅', parse_mode='html', reply_markup=InlineKeyboardMarkup(keyboard=keys1))
        else:
            bot.answer_callback_query(call.id, "Only admins can perform this action", show_alert=True)

    
    elif call.data.startswith('ofwel'):
        admin_id = call.data.split('_')[1].strip()
        chatID = call.message.chat.id
        if call.from_user.id == int(admin_id):
            current_status_welcome = call.data.replace('ofwel', '').split('_')[0].strip()
            group_name_gu = bot.get_chat(chatID).title
            if current_status_welcome == 'on':
                bot.edit_message_text(chat_id=chatID, message_id=call.message.message_id, text='<b>Are you sure you want to turn off welcome message?</b>\n\nClick here to save changes', parse_mode='html', reply_markup=InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("Save changes ✅", callback_data=f'sweloff_{admin_id}'), InlineKeyboardButton("<< Back", callback_data=f'wel_{admin_id}')))
            else:
                bot.answer_callback_query(call.id, text="Welcome message is already off", show_alert=True)
        else:
            bot.answer_callback_query(call.id, "Only admins can perform this action", show_alert=True)

    elif call.data.startswith('onwel'):
        admin_id = call.data.split('_')[1].strip()
        chatID = call.message.chat.id
        if call.from_user.id == int(admin_id):
            group_name_welcome = bot.get_chat(chatID).title
            current_status_welcome = call.data.replace('onwel', '').split('_')[0].strip()
            if current_status_welcome == 'off':
                bot.edit_message_text(chat_id=chatID, message_id=call.message.message_id, text='<b>Are you sure you want to turn on welcome message?</b>\n\nClick here to save changes', parse_mode='html', reply_markup=InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("Save changes ✅", callback_data=f'swelon_{admin_id}'), InlineKeyboardButton("<< Back", callback_data=f'wel_{admin_id}')))
            else:
                bot.answer_callback_query(call.id, text="Welcome message is already on", show_alert=True)
        else:
            bot.answer_callback_query(call.id, "Only admins can perform this action", show_alert=True)
    elif call.data.startswith('changewel'):
        admin_id = call.data.replace('changewel_', '').strip()
        chatID = call.message.chat.id
        if call.from_user.id == int(admin_id):
            to_send = '''
To change the welcome message send
<code>/welcome &lt;New message&gt;</code>.

To specify the joined user's name:
Use [NAME] in the message
eg:
<code>/welcome Welcome [NAME]!</code>

To specify group's name:
Use [GROUPNAME] in the message
eg:
<code>/welcome Welcome to [GROUPNAME]</code>

Example usage:
<code>/welcome Welcome [NAME] to [GROUPNAME]</code>
'''
            bot.edit_message_text(chat_id=chatID, message_id=call.message.message_id, text=to_send, parse_mode='html', reply_markup=InlineKeyboardMarkup().add(InlineKeyboardButton("<< Back", callback_data=f'wel_{admin_id}')))
        else:
            bot.answer_callback_query(call.id, "Only admins can perform this action", show_alert=True)

    ############################# WELCOME MESSAGE SETTINGS - (ENDS HERE) #############################
...
@bot.message_handler(commands=['welcome'], chat_types=['group', 'supergroup'], is_chat_admin=True)
def set_welcome(message):
    chatID = message.chat.id
    db = groups_collection.find_one({'id':str(chatID)})
    if db['general']['welcome'] == 'on':
        group_name = bot.get_chat(chatID).title

        query = message.text.replace('/welcome', '').strip()
        if db['general']['clean_mode'] == 'on':
            bot.delete_message(chat_id=message.chat.id, message_id=message.message_id)
        current_message = db['general']['welcome_message']

        if len(query) == 0:
            bot.send_message(chatID, f"<b>Current welcome message :</b>\n\n{current_message}\n\n<b>To set a new one:</b>\n\n`<code>/welcome Welcome [NAME] to [GROUPNAME]. Enjoy your stay :)</code>`", parse_mode='html')
        else:
            if db:
                try:
                    groups_collection.update_one({'id':str(chatID)}, {'$set':{'general.welcome_message':query}})
                    bot.send_message(chat_id=chatID, text=f"Successfully updated the welcome message for {group_name} ✅")
                except:
                    bot.send_message(chatID, "Operation failed, Make the bot admin.")
            else:
                bot.send_message(chatID, "Operation failed, Make the bot admin.")
    else:
        bot.send_message(chatID, "Cannot change welcome message when welcome message is turned off.\n\nTo turn on:\n\n_/settings > General > Welcome > On_", parse_mode='Markdown')
...
def welcome(message):
    if int(message.new_chat_members[0].id) != int(bot.get_me().id):
        chatID = message.chat.id
        db = groups_collection.find_one({'id':str(chatID)})
        if db['general']['welcome'] == 'on':
            name = message.new_chat_members[0].first_name
            group_name = bot.get_chat(chatID).title
            welcome_message = db['general']['welcome_message'].replace('[NAME]', name).replace('[GROUPNAME]', group_name)
