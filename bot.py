...
def callback_query_handler(call):
    if call.data.startswith('hehe'):
        ...
        markup = InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("ʜɪɢʜ ʀᴇꜱᴏʟᴜᴛɪᴏɴ", callback_data="highres"), InlineKeyboardButton("ʟᴏᴡ ʀᴇꜱᴏʟᴜᴛɪᴏɴ", callback_data="lowres"), InlineKeyboardButton("﹤﹤ ʙᴀᴄᴋ", callback_data="home_yt"))
...
def developer(message):
    if message.chat.type == 'group' or message.chat.type == 'supergroup':
        ...
        if config_cleanmode == 'on':
            bot.delete_message(message.chat.id, message.message_id)
    keys = InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("Github", url="https://github.com/47hxl-53r"), InlineKeyboardButton("Telegram", url="https://t.me/ieatkidsforlunch"), InlineKeyboardButton("Buy me a coffee?", url="https://buymeacoffee.com/mortylabz"))
...
def youtube_command(message):
    ...
    if len(query) == 0:
        ...
        if isValid(youtubevideourl):
            ...
            markup = InlineKeyboardMarkup(row_width=2).add(InlineKeyboardButton("ᴠɪᴅᴇᴏ 📹", callback_data= "video"), InlineKeyboardButton("ᴀᴜᴅɪᴏ 🎧", callback_data="audior"), InlineKeyboardButton("❌ ᴄᴀɴᴄᴇʟ ❌", callback_data=f"cls_{message.chat.id}"))
...
def reply(message):
    ...
    markup = InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("ʜɪɢʜ ʀᴇꜱᴏʟᴜᴛɪᴏɴ", callback_data=f"hgroup"), InlineKeyboardButton("ʟᴏᴡ ʀᴇꜱᴏʟᴜᴛɪᴏɴ", callback_data=f'lgroup'), InlineKeyboardButton("❌ ᴄʟᴏꜱᴇ ❌", callback_data="close_yt"))  
