...
# /PLAY COMMAND
@bot.message_handler(commands=['play'], chat_types=['private', 'group', 'supergroup'])
    ...
    if collection_users.find_one({"id":str(userID)}):
        ...
        if len(photos.photos) >= 1:
            ...
            with open('default.jpg', 'rb') as image:
                bot.delete_message(chat_id=message.chat.id, message_id=fetch.message_id)
                bot.send_photo(chat_id=message.chat.id, photo=image, caption=caption, parse_mode='html', reply_markup=markup)
    else:
        if message.chat.type == 'private':
