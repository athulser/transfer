...
def split_by_manifest(filename, manifest, output_dir, vcodec="copy", acodec="copy",
                      extra="", **kwargs):
    ...
    Arguments:
        ...
        output_dir (str)    - Output directory for the segmented video files.
    ...
    with open(manifest) as manifest_file:
        ...
        for video_config in config:
            ...
            try:
                ...
                output_file_path = os.path.join(output_dir, filebase + "." + fileext)
                split_args += ["-ss", str(split_start), "-t",
                               str(split_length), output_file_path]
                print("########################################################")
...
def split_by_seconds(filename, split_length, output_dir, vcodec="copy", acodec="copy",
    ...
    for n in range(0, split_count):
        ...
        output_file_path = os.path.join(output_dir, filebase + "-" + str(n + 1) + "-of-" +
                       str(split_count) + "." + fileext)
        split_args += ["-ss", str(split_start), "-t", str(split_length), output_file_path]
        subprocess.check_output(split_cmd + split_args)
...
def main():
    ...
    parser.add_option("-o", "--output-dir",
                      dest="output_dir",
                      help="Output directory for the segmented video files.",
                      type="string",
                      default="./",
                      action="store"
                      )
    ...
    if options.manifest:
        split_by_manifest(options.filename, options.manifest, options.output_dir,
        ...
        if not options.split_length:
            bailout()
        split_by_seconds(options.filename, options.split_length, options.output_dir,
                         vcodec=options.vcodec, acodec=options.acodec,
                         extra=options.extra, video_length=video_length)
